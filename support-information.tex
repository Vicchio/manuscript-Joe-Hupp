...
\newpage
\subsection{Ni-O Coordination Numbers}
...
\newpage
\subsection{ Gas Phase Ni4 Cluster Models}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-cluster-gas-phase-calculation-schematic.png}
    \caption{
        Atomic drawings showing the conversion a periodic structure to a cluster structure for DFT optimization. The cluster structure represents a gas phase DFT calculation that we tested in CP2K, given none of the framework being included in the calculation and the unit cell is expanded to a 50 {\AA} x 50 {\AA} x 50 {\AA} cubic box. The pyrene linkers were truncated and capped with H atoms, as described by the formate cluster model.\cite{Bernales2016} The node structures are fixed entirely here, only allowing the \ce{Ni4} cluster to relax. This is the same procedure we used when performing the frequency calculations in the periodic calculations. 
    }
    \label{fig:SI-cluster-gas-phase-schematic}
\end{figure}
...
\centering
  \setlength\tabcolsep{8pt}
  \caption{Comparing Periodic and Gas Phase Reaction Energies}
  \label{tbl:alternate_model_energies_gas_phase}
  \begin{tabular}{lcccc}
    \hline
        Structure & $N_{\ce{H2}}$ & $N_{\ce{H2O}}$ & \thead{$\Delta E_{\text{periodic}}$ \\ (kJ/mol)\textsuperscript{a}} & \thead{$\Delta E_{\text{gas}}$ \\ (kJ/mol)\textsuperscript{a}} \\
        \hline
        a) \ce{Ni4(OH)6.4H2O} (green)    &      0     &       0       &    0.0  &    0.0 \\
        b) \ce{Ni4(OH)6.3H2O} (purple)   &      0     &       1       &  122.2  &   83.4 \\
        c) \ce{Ni4(OH)6.2H2O} (cyan)     &      0     &       2       &  203.5  &  172.3 \\
        d) \ce{Ni4(OH)5(H).4H2O} (blue)  &      1     &       0       &  -91.0  &  -92.0 \\
        \hline
    \end{tabular} \\
    \textsuperscript{a}$\Delta E$ = $(E_{\ce{Ni4(OH)_{x}.yH2O}} + N_{\ce{H2O}} * E_{\ce{H2O}}) - ( E_{\ce{Ni4(OH)6.4H2O}} + N_{\ce{H2}} * E_{\ce{H2}})$ \\
\end{table}    
\end{center}

The conversion from a periodic \ce{Ni4} cluster to a gas phase \ce{Ni4} cluster is shown in Figure~\ref{fig:SI-cluster-gas-phase-schematic}. The electronic structure parameters for the gas phase \ce{Ni4} cluster calculations is the same as the computational parameters presented in SI S1.8. The unit cell for the gas phase parameters is expanded to a 50 {\AA} x 50 {\AA} x 50 {\AA} cubic box to represent the gas phase conditions. Table~\ref{tbl:alternate_model_energies_gas_phase} shows the $\Delta E_{\text{periodic}}$ calculated from periodic DFT calculations in CP2K and the $\Delta E_{\text{gas}}$ computed from the gas phase CP2K calculations. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Variable Ni Content Phase Diagram}
\begin{figure}[H]
    \centering
...
All thermodynamic minima structures according to \textit{ab initio} thermodynamic analysis are presented in Figures~\ref{fig:SI-dPDF-green} to~\ref{fig:SI-dPDF-red}. Each figure includes the (a) fixed unit cell d-PDFs, (b) the schematic drawing, and (c) the atomic rendering for that structure. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Cluster Relative Energies}

We compute the free energy landscape of \ce{Ni4(OH)5(H).4H2O} (blue) and \ce{Ni4(OH)6.3H2O} (purple), shown in Figures~\ref{fig:SI-flux-blue} and \ref{fig:SI-flux-purple} respectively. For the same stoichiometries of \ce{Ni_4O_xH_y}, the relative free energy ($F$), as described in the \textit{ab initio} Thermodynamics Modeling section of the main text, is computed for each structure and referenced to the phase diagram structure. Figure~\ref{fig:SI-flux-blue} reveals several structures within 50 kJ/mol of \ce{Ni4(OH)5(H).4H2O} (blue), whereas Figure~\ref{fig:SI-flux-purple} shows no structures within 300 kJ/mol of \ce{Ni4(OH)6.3H2O} (purple).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.50\textwidth]{zi-images/04-SI-images/2022-SI-flux-blue.png}
    \caption{
        The relative free energy referenced to the \ce{Ni4(OH)5(H).4H2O} (blue) structure at standard conditions. Alternative structures are plausible for this structure, suggesting that fluxionality may be important for this particular structure. 
    }
...
\begin{figure}[H]
    ...
    \caption{
        The relative free energy referenced to the \ce{Ni4(OH)6.3H2O} (purple) structure at standard conditions. Alternative structures are not plausible for this structure, suggesting that fluxionality probably is not important for this particular structure on the phase diagram. Note that these structures are within 100 kJ/mol of each other electronically, but including the vibrational contributions results in the large differences in free energy.
    }
...
\centering
  \setlength\tabcolsep{8pt}
  \caption{Comparing Periodic and DFT + U Reaction Energies}
  \label{tbl:alternate_model_energies_DFTU}
  \begin{tabular}{lcccc}
    \hline
        Structure & $N_{\ce{H2}}$ & $N_{\ce{H2O}}$ & \thead{$\Delta E_{\text{periodic}}$ \\ (kJ/mol)\textsuperscript{a}} & \thead{$\Delta E_{\text{DFT + U}}$ \\ (kJ/mol)\textsuperscript{a}} \\
        \hline
        a) \ce{Ni4(OH)6.4H2O} (green)    &      0     &       0       &    0.0  &   0.0 \\
        b) \ce{Ni4(OH)6.3H2O} (purple)   &      0     &       1       &  122.2  &  71.6 \\
        c) \ce{Ni4(OH)6.2H2O} (cyan)     &      0     &       2       &  203.5  &  42.1 \\
        d) \ce{Ni4(OH)5(H).4H2O} (blue)  &      1     &       0       &  -91.0  &  10.9 \\
        \hline
    \end{tabular} \\
    \textsuperscript{a}$\Delta E$ = $(E_{\ce{Ni4(OH)_{x}.yH2O}} + N_{\ce{H2O}} * E_{\ce{H2O}}) - ( E_{\ce{Ni4(OH)6.4H2O}} + N_{\ce{H2}} * E_{\ce{H2}})$ \\
\end{table}    
...
\begin{figure}[H]
    ...
    \caption{
        Calculated phase diagram for the \ce{Ni4} cluster presented as a function $\Delta \mu_{\ce{H2}}$ and $\Delta \mu_{\ce{H2O}}$ with the DFT + U correction term implemented. The correct terms (U = 6.8667 and J = 1.23958 for \ce{NiO2}) are from the \textit{Database of Hubbard U and J Values for TMOs}\cite{Hubbard_parameter_website} according to\citeauthor{Moore2022} The conditions for $\Delta \mu_{\ce{H2}}$ and $\Delta \mu_{\ce{H2O}}$ are expanded from the conditions presented in Figure~4 (main text). The different colored regions indicate different structures, following the same key as in Figure~\ref{fig:SI-all-dPDFs}. The vertical dashed line indicates the value of $\Delta \mu_{\text{H}_2}$ corresponding to the conditions where the experimental d-PDF was collected.\cite{PlateroPrats2016}
    }
    \label{fig:SI-phase_diagram_DFTU}
\end{figure}

We evaluate the performance of the Hubbard model DFT + U scheme for the \ce{Ni4} clusters by including the Hubbard terms for the \ce{Ni} atoms within the system. We selected correction terms for \ce{NiO2} (U = 6.8667 eV and J = 1.23958 eV) from the \textit{Database of Hubbard U and J Values for TMOs}\cite{Hubbard_parameter_website} as formulated by \citeauthor{Moore2022} We add terms to the \ce{Ni} species within the CP2K input file and perform single-point energy calculations for all structures on the phase diagram. Geometry optimizations were not feasible in CP2K with the \textit{PLUS\_U\_METHOD LOWDIN} selection; however, we could not converge the SCF when using the \textit{PLUS\_U\_METHOD MULLIKEN}. The results presented in Table~\ref{tbl:alternate_model_energies_DFTU} and Figure~\ref{fig:SI-phase_diagram_DFTU} are from single point energy calculations only involving the DFT + U method; geometry optimizations with DFT + U were not feasible. 

Starting from the converged geometries of the structures appearing on phase diagram (Figure~4 of the main text), the electronic energy was recalculated with the DFT + U formalism to evaluate the influence of U on the phase diagram structures. The reaction energies ($\Delta E$) are computed in Table~\ref{tbl:alternate_model_energies_DFTU} for the periodic and DFT + U electronic energies. Large differences in Table~\ref{tbl:alternate_model_energies_DFTU} are observed between $\Delta E_{\text{periodic}}$ and $\Delta E_{\text{DFT + U}}$. These differences cannot be separated into contributions from the +U method versus the energies were computed from unconverged structures (due to now implementation the +U method). However, we still construct a phase diagram with these energies from DFT + U combined with the vibrational free energy term that does not provide any new observations or conclusions (Figure~\ref{fig:SI-phase_diagram_DFTU}). The conditions for $\Delta \mu_{\ce{H2}}$ and $\Delta \mu_{\ce{H2O}}$ are expanded from the Figure~4 to evaluate the influence of DFT + U on the phase diagram. A few structures at low \ce{Ni-O} coordination numbers are removed (\ce{Ni4(OH)4} (white) and \ce{Ni4(OH)3(H).H2O} (magenta)). However, all structures with \ce{Ni-O} above 3.25 remain, which are the structures that are in better agreement based on the experimentally observed \ce{Ni-O} coordination numbers. 
...
\centering
  ...
  \begin{tabular}{lcccc}
    \hline
        ...
        l) \ce{Ni4(OH)4(O).3H2O} (teal)    &  40.583 & 16.014 \\
        \hline
    \end{tabular} \\
...
\centering
  ...
  \begin{tabular}{lcccc}
    \hline
        ...
        l) \ce{Ni4(OH)4(O).1H2O} (teal)   & -4794.9465 & -4794.9466 &  0.255 & \\
        \hline
    \end{tabular} \\
    \textsuperscript{a}unit cell parameters are fixed at $a=b=40.611$ {\AA}, $c=15.990$ {\AA}, $\alpha=\beta=\ang{90}$, and $\gamma=\ang{60}$ \\
    \textsuperscript{b}unit cell parameters $a=b$ and $c$ are varied with $\alpha=\beta=\ang{90}$ and $\gamma=\ang{60}$ fixed
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-green.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)6.4H2O} (green) with variable unit cell (solid green) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_green}
\end{figure}

% purple
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-purple.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)6.3H2O} (purple) with variable unit cell (solid purple) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_purple}
\end{figure}

% Blue
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-blue.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)5(H).4H2O} (blue) with variable unit cell (solid blue) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_blue}
\end{figure}

% Cyan
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-cyan.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)6.2H2O} (cyan) with variableunit cell (solid cyan) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_cyan}
\end{figure}

% yellow
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-yellow.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)4(O)} (yellow) with variable unit cell (solid yellow) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_yellow}
\end{figure}

% Orange
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-orange.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)4.2H2O} (orange) with variable unit cell (solid orange) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_orange}
\end{figure}

% White
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-white.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)4} (white) with variable unit cell (solid white) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_white}
\end{figure}

% magenta
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-magenta.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)3(H).H2O} (magenta) with variable unit cell (solid magenta) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_magenta}
\end{figure}

% red
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-red.png}
    \caption{
        The d-PDFs for \ce{Ni4(H)4} (red) with variable unit cell (solid red) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_red}
\end{figure}

% gray
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-gray.png}
    \caption{
        The d-PDFs for \ce{Ni4} (gray) with variable unit cell (solid gray) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_gray}
\end{figure}

% maroon
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-maroon.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)4(O).3H2O} (maroon) with variable unit cell (solid maroon) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_maroon}
\end{figure}

% teal
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-dPDF-compare-teal.png}
    \caption{
        The d-PDFs for \ce{Ni4(OH)4(O).1H2O} (teal) with variable unit cell (solid teal) and fixed unit cell (black), where the peaks of the variable unit cell (black) are labeled.
    }
    \label{fig:SI-dPDF_unit_cell_teal}
\end{figure}

We allow the unit cell to relax for all structures appearing on the phase diagram, Figure 4 (main text), as well as the \ce{Ni4(OH)4(O).3H2O} (maroon) structure. Table~\ref{tbl:unit_cell_optimization_cell_parameters} and~\ref{tbl:unit_cell_optimization_energies} show the unit cell parameters and the energies for these unit cell optimizations. We observe negligible differences in both the unit cell parameters and the energies after the cell optimizations. Furthermore, the d-PDFs these new optimized structures are reported in Figures~\ref{fig:SI-dPDF_unit_cell_green} to \ref{fig:SI-dPDF_unit_cell_teal}; similarly, negligible changes in the d-PDF peak positions are observed when optimizing the unit cell parameters (black line) compared to the original the peak positions of the fixed unit cell (solid, colored line) for all structures. 


% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %% Water Movement Check (node to cluster)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \newpage
% \section{ Energetics of Moving a Node Water Ligand from the Node to Cluster}

% \begin{center}
% \begin{table}[H]
% \centering
%   \setlength\tabcolsep{8pt}
%   \caption{Water Selection Configuration}
%   \label{tbl:water_selection_energies_purple_alt}
%   \begin{tabular}{lcc}
%     \hline
%         Structure &  \thead{Electronic Energy \\ (hartree)} & \thead{$\Delta$Electronic \\ Energy (kJ/mol)} \\
%         \hline
%         \ce{Ni4(OH)6.3H2O} (purple)             & -4829.4227 &  0.0 \\
%         \ce{Ni4(OH)6.4H2O}-config\_1 (green)    & -4829.3949 & 73.2 \\
%         \ce{Ni4(OH)6.4H2O}-config\_2 (green)    & -4829.4059 & 44.3 \\
%         \hline
%     \end{tabular} \\
%     % \textsuperscript{a}unit cell parameters are fixed at $a=b=40.611$ {\AA}, $c=15.990$ {\AA}, $\alpha=\beta=\ang{90}$, and $\gamma=\ang{60}$ \\
%     % \textsuperscript{b}unit cell parameters $a=b$ and $c$ are varied with $\alpha=\beta=\ang{90}$ and $\gamma=\ang{60}$ fixed
...
%   \setlength\tabcolsep{8pt}
%   \caption{Water Selection Configuration}
%   \label{tbl:water_selection_energies_cyan_alt}
%   \begin{tabular}{lcc}
%     \hline
%         Structure &  \thead{Electronic Energy \\ (hartree)} & \thead{$\Delta$Electronic \\ Energy (kJ/mol)} \\
%         \hline
%         \ce{Ni4(OH)6.2H2O} (cyan)                & -4812.1716 &  0.0 \\
%         \ce{Ni4(OH)6.3H2O}-config\_1 (purple)    & -4812.1410 & 80.1 \\
%         \ce{Ni4(OH)6.3H2O}-config\_2 (purple)    & -4812.1396 & 83.9 \\
%         \hline
%     \end{tabular} \\
%     % \textsuperscript{a}unit cell parameters are fixed at $a=b=40.611$ {\AA}, $c=15.990$ {\AA}, $\alpha=\beta=\ang{90}$, and $\gamma=\ang{60}$ \\
%     % \textsuperscript{b}unit cell parameters $a=b$ and $c$ are varied with $\alpha=\beta=\ang{90}$ and $\gamma=\ang{60}$ fixed
...
% Table~\ref{tbl:water_selection_energies_purple_alt} and \ref{tbl:water_selection_energies_cyan_alt} show the relative energies of structures with the same overall stoichiometry, but different \ce{Ni_4} clusters. \ce{H2O}-ligands were moved from adjacent NU-1000 nodes to increase the \ce{H2O}-ligands present on the cluster (i.e., \ce{Ni4(OH)6.3H2O} (purple) is converted to \ce{Ni4(OH)6.4H2O}-config\_1 (green) and \ce{Ni4(OH)6.4H2O}-config\_2 (green), and \ce{Ni4(OH)6.2H2O} (cyan) is converted to \ce{Ni4(OH)6.3H2O}-config\_1 (purple) and \ce{Ni4(OH)6.3H2O}-config\_2 (purple)). Both Table~\ref{tbl:water_selection_energies_purple_alt} and~\ref{tbl:water_selection_energies_cyan_alt} show that moving an \ce{H2O}-ligand from the MOF node to increase the \ce{H2O}-ligand content on the \ce{Ni4} cluster is thermodynamically unfavorable. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Free Water Movement Check (node to cluster)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{ Energetics of Moving a Free Water Ligand From Node to Cluster}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{zi-images/04-SI-images/2022-SI-H2O_node_to_cluster.png}
    \caption{
        A schematic drawing showing the interconversion from a hydrogen bonded \ce{H2O} on an adjacent \ce{OH}/\ce{H2O} pair to the \ce{Ni4} cluster. Note that the hydrogen bonded \ce{H2O} ligand is not shown. The different configurations shown in Tables~\ref{tbl:water_selection_energies_green} to \ref{tbl:water_selection_energies_teal} represent the different initial configurations for \ce{H2O} ligand placement on the node. 
    }
    \label{fig:SI-H2O_node_to_ligand}
\end{figure}

The \ce{H2O} transfer energy ($\Delta E_{\text{transfer}}$), which represents the energy of \ce{H2O} ligand transferring from the MOF node to the \ce{Ni4} cluster, is computed according to Eq.~\ref{eq:h2o_transfer_energy}:
\begin{equation}
    \begin{split}
        [\text{MOF Node with } \ce{H2O}] + [\ce{Ni4} & \text{ Cluster missing } \ce{H2O}]   \\
        & \ce{->[$\Delta E_{\text{transfer}}$]} [\ce{Ni4} \text{ Cluster with } \ce{H2O}] + [\text{MOF Node missing } \ce{H2O}]     
    \end{split}
    \label{eq:h2o_transfer_energy}
\end{equation}

The \ce{H2O} binding energy of a \ce{H2O} to the MOF node ($\Delta E_{\ce{H2O}}$) is computed according to Eq.~\ref{eq:h2o_binding_energy}:
\begin{equation}
     [\text{MOF Node missing } \ce{H2O}] + \ce{H2O} \ce{->[$\Delta E_{\text{\ce{H2O}}}$]} [\text{MOF Node with } \ce{H2O}]
    \label{eq:h2o_binding_energy}
\end{equation}

Using the electronic energies presented in Tables~\ref{tbl:water_selection_energies_green} and ~\ref{tbl:water_selection_energies_purple}, which are for the \ce{Ni4(OH)6.4H2O} (green) and \ce{Ni4(OH)6.3H2O} (purple) structures appearing on the phase diagram respectively, the $\Delta E_{\text{transfer}}$ and $\Delta E_{\ce{H2O}}$ are computed. The findings are presented in Table~\ref{tbl:water_selection_energies_binding_transfer}. Additionally, we also computed the $\Delta E_{\ce{H2O}}$ for the \ce{Ni4(OH)6.1H2O} (teal) structure, which does not appear on the phase diagram as it is not a thermodynamic minima under any of the conditions explored, with the electronic energies for \ce{Ni4(OH)6.1H2O} (teal) are presented in Table~\ref{tbl:water_selection_energies_teal}. 

\begin{center}
\begin{table}[H]
\centering
  \setlength\tabcolsep{5pt}
  \caption{Electronic Energies for Alternate \ce{H2O}-ligands for \ce{Ni4(OH)6.4H2O} (green)}
  ...
  \begin{tabular}{lcc}
    \hline
        ...
        \thead{$E$ \\ (hartree)} \\
        \hline
        \ce{Ni4(OH)6.4H2O} (green)                       &   \ce{Ni4} Cluster with \ce{H2O}     & -4846.6895  \\ 
        \ce{Ni4(OH)6.3H2O} (purple)                      &   \ce{Ni4} Cluster missing \ce{H2O}  & -4829.4227  \\ 
        \ce{Ni4(OH)6.3H2O}-alt\textsuperscript{a}        &   MOF Node with \ce{H2O}             & -4846.6864  \\ 
        \ce{Ni4(OH)6.3H2O}-r\ce{H2O}\textsuperscript{b}  &   MOF Node missing \ce{H2O}          & -4829.4444  \\ 
        \hline
    \end{tabular} \\
    \textsuperscript{a}The -alt indicates an \ce{H2O} ligand placed on an adjacent node. \\
    \textsuperscript{b}The -r\ce{H2O} indicates the removal of an \ce{H2O} ligand placed on an adjacent node. 
\end{table}    
\end{center}
...
\centering
  \setlength\tabcolsep{5pt}
  \caption{Electronic Energies for Alternate \ce{H2O}-ligands for \ce{Ni4(OH)6.3H2O} (purple)}
  ...
  \begin{tabular}{lcc}
    \hline
        ...
        \thead{$E$ \\ (hartree)} \\
        \hline
        \ce{Ni4(OH)6.3H2O} (purple)                      &   \ce{Ni4} Cluster with \ce{H2O}       & -4829.4227  \\ 
        \ce{Ni4(OH)6.2H2O} (cyan)                        &   \ce{Ni4} Cluster missing \ce{H2O}    & -4812.1716  \\ 
        \ce{Ni4(OH)6.2H2O}-alt\textsuperscript{a}        &   MOF Node with \ce{H2O}               & -4829.4364  \\
        \ce{Ni4(OH)6.2H2O}-r\ce{H2O}\textsuperscript{b}  &   MOF Node missing \ce{H2O}            & -4812.1981  \\ 
        \hline
    \end{tabular} \\
    \textsuperscript{a}The -alt indicates an \ce{H2O} ligand placed on an adjacent node. \\
    \textsuperscript{b}The -r\ce{H2O} indicates the removal of an \ce{H2O} ligand placed on an adjacent node.
...
\centering
  \setlength\tabcolsep{8pt}
  \caption{Electronic Energies for Alternate \ce{H2O}-ligands for \ce{Ni4(OH)6.1H2O} (teal)}
  ...
  \begin{tabular}{lcc}
    ...
    \thead{$E$ \\ (hartree)} \\
        \hline
        \ce{Ni4(OH)6.1H2O} (teal)                    &   \ce{Ni4} Cluster with \ce{H2O}       & -4794.9465  \\ 
        \ce{Ni4(OH)6}-alt\textsuperscript{a}         &   MOF Node with \ce{H2O}               & -4794.9433  \\
        \ce{Ni4(OH)6}-r\ce{H2O}\textsuperscript{b}   &   MOF Node missing \ce{H2O}            & -4777.6995  \\ 
        \hline
    \end{tabular} \\
    \textsuperscript{a}The -alt indicates an \ce{H2O} ligand placed on an adjacent node. \\
    \textsuperscript{b}The -r\ce{H2O} indicates the removal of an \ce{H2O} ligand placed on an adjacent node. 
...
\centering
  \setlength\tabcolsep{8pt}
  \caption{Computed $\Delta E_{\text{transfer}}$ and $\Delta E_{\text{\ce{H2O}}}$}
  \label{tbl:water_selection_energies_binding_transfer}
  \begin{tabular}{lcc}
    \hline
    Structure &
    \thead{$\Delta E_{\text{transfer}}$ \\ (kJ/mol)\textsuperscript{a}} &
    \thead{$\Delta E_{\text{\ce{H2O}}}$ \\ (kJ/mol)\textsuperscript{b}} \\
        \hline
           \ce{Ni4(OH)6.4H2O} (green)     &     -65.0    &  -57.2 \\ 
           \ce{Ni4(OH)6.3H2O} (purple)    &     -34.0    &  -47.2 \\
           \ce{Ni4(OH)6.1H2O} (teal)      &       -      &  -62.1   \\ 
        \hline
    \end{tabular} \\
    \textsuperscript{a}Computed according to Eq.~\ref{eq:h2o_transfer_energy} \\
    \textsuperscript{b}Computed according to Eq.~\ref{eq:h2o_binding_energy} 
\end{table}    
\end{center}

Table~\ref{tbl:water_selection_energies_binding_transfer} shows the $\Delta E_{\text{transfer}}$ and the $\Delta E_{\text{\ce{H2O}}}$ for a single, hydrogen bonded \ce{H2O}-ligand was moved from the \ce{OH}/\ce{H2O} ligand pairs on an adjacent node to the \ce{Ni4} cluster to capture the thermodynamics of \ce{H2O}-ligands interchanging from the node and the cluster. We simulate different configurations to account for \ce{H2O}-ligand placement on the node (Figure~\ref{fig:SI-H2O_node_to_ligand}), but only present the lowest energy configurations (denoted -alt) is considered. We also consider removing this single, hydrogen bonded \ce{H2O} ligand (denoted -r\ce{H2O}). The results presented in Table~\ref{tbl:water_selection_energies_binding_transfer} suggest that in some circumstances \ce{H2O} moving from the node to the cluster is thermodynamically favorable given that the average $E_{\text{transfer}}$ is -53.7 kJ/mol, and the average $E_{\text{\ce{H2O}}}$ for a free \ce{H2O} ligand hydrogen bonding the accessible \ce{-OH}/\ce{-OH2} ligands to the cluster is -52.2 kJ/mol. 
