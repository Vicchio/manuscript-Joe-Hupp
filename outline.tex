...
\begin{figure}[H]
    ...
    \caption{
    Differential pair distribution functions (dPDFs) capturing the local structural information of the \ce{Ni} cluster. The (a) experimental dPDF, (b) select thermodynamic minima from \textit{ab initio} thermodynamic modeling structures, and (c) a select non-thermodynamic minima structure are reported. The \ce{Ni-O}, \ce{Ni{\Compactcdots}Ni}, and \ce{Ni{\Compactcdots}Zr} distances of the experimental are indicated by the gray dashed lines, enabling comparisons in peak positions between the experimental and model structures. The color scheme is the same as in Figures~\ref{fig:phase_diagram_Ni_combined} and \ref{fig:Ni-structure-diagram}.
    }
    \label{fig:dPDFs_TandP_trans_Ni}
\end{figure}

The experimental result from dPDF analysis is shown in Figure~\ref{fig:dPDFs_TandP_trans_Ni} (a), where the dPDF of the experimentally observed structure exposed to \ce{H2} gas is depicted with the black line. \cite{PlateroPrats2017} Key peaks in the experimentally observed dPDF are the single peak at 2.02 {\AA}, split peaks at 3.01 {\AA} and 3.27 {\AA}, and split peaks at 3.78 {\AA} and 4.08 {\AA}. These correspond to \ce{Ni-O}, \ce{Ni{\Compactcdots}Ni}, and \ce{Ni{\Compactcdots}Zr} distances, respectively. Our analysis primarily focuses on the \ce{Ni-O} and \ce{Ni{\Compactcdots}Ni} seen in the experimental dPDF; we evaluate the performance of a structure by how well it recreates the peak features observed by the experimental dPDF. Furthermore, the \ce{Ni-O} coordination number of the activated structure is measured to be $\sim$5 under activation conditions in \ce{H2}. We use the following information as a reference during \textit{ab initio} thermodynamic analysis to identify appropriate \ce{H2} and \ce{H2O} chemical potential values.  
...
\begin{figure}[H]
    ...
    \caption{
    Depiction of key structures considered in this work from (a) \textit{ab initio} thermodynamic modeling structures and (b) select non-thermodynamic minima. The color scheme is the same as in Figures~\ref{fig:phase_diagram_Ni_combined} and \ref{fig:dPDFs_TandP_trans_Ni}. All \ce{H2O}s are colored blue, and any Nickel hydride species (\ce{Ni-H}) are colored red. The color scheme is the same as in Figures~\ref{fig:phase_diagram_Ni_combined} and \ref{fig:dPDFs_TandP_trans_Ni}.
    }
    \label{fig:Ni-structure-diagram}
\end{figure}

% paragraph on the structure with good coordination from the structure diagrams
The structures from \textit{ab initio} thermodynamic analysis that minimize the free energy expression (Eq. \ref{eq:free-energy-trans}) and exhibit \ce{Ni-O} coordination numbers above 3.3 are shown in Figure \ref{fig:Ni-structure-diagram} (a) and appear on the phase diagram (Figure~\ref{fig:phase_diagram_Ni_combined}). We order the structures according to the \ce{Ni-O} coordination numbers, determined by inspecting the \ce{Ni-O} bond distances within each the structure. All structures appearing on the phase diagram (Figure~\ref{fig:phase_diagram_Ni_combined}) are located within the \hl{Supporting Information.} 





We inspect the \ce{Ni-O} bond distances to compute the \ce{Ni-O} coordination number, and order the structures appearing in Figure \ref{fig:Ni-structure-diagram} accordingly. 

\hl{statement here about the structures that show good agreement in terms of coordination number...and then the ones that don't}.
...
% Paragraph on the phase diagram here with the structures that are good 
...
% % % % % % % % % % OLD TEXT SHOWN BELOW HERE % % % % % % % % % %

Now, the dPDFs shown in Figure~\ref{fig:dPDFs_TandP_trans_Ni} represent the structures that minimize the free energy according to \textit{ab initio} thermodynamic analysis. However, before focusing on the conditions where the structures are located, we first inspect the \ce{Ni} coordination numbers of the structures. \hl{Platero-Prats et al.} observed that the coordination number of the experimental dPDFs was thought to be 5-ish under activation conditions. We report the \ce{Ni} coordination numbers on the \hl{right side?} of Figure~\ref{fig:dPDFs_TandP_trans_Ni}. The structures that contain \ce{Ni} coordination numbers that deviate from the experimental observed coordination numbers, such as structures \hl{XXX}, \hl{XXX}, and \hl{XXX}, show little to no agreement in \ce{Ni-O} or \ce{Ni{\Compactcdots}Ni} peak positions. Therefore, we use coorindation numbers in defining the chemical potentials used in \textit{ab initio} thermodynamic analysis. 

Now, we provide insights into the types of structures that could be observed experimentally at relevant compositions (i.e., structures with reasonable coordination numbers) by constructing phase diagrams as a function of \hl{the chemical potential terms} (Figure~\ref{fig:phase_diagram_Ni_combined}).
...
\begin{itemize}
    \item Li sintering... the \ce{Ni-H} might exist under a transient state according to EXAFS.. which the model is further supporting here given we do not observe a lot of structure that feature a \ce{Ni-H} here (revisit the Sintering paper for further review, analysis) 
\end{itemize}
