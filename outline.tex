...
To compute $E^\text{ZP}$ and $F^\text{vib}$, the vibrational modes are first calculated using the CP2K software package. At each unique composition, we compute the vibrational modes for structures within 100 kJ/mol of lowest energy structure at that configuration, which reduced the number of structures from \hl{XXX} to \ce{XXX} for vibrational mode analysis. We fix the organic linker and inorganic nodes, thereby only calculating only the \ce{Ni} cluster vibrational contributions to the free energy ($F^\text{vib}$). We ensure that the same atoms are fixed across all frequency calculations in order to correctly compare ($F^\text{vib}$). When constructing the vibrational partition function, all frequencies less than 50 cm\textsuperscript{-1} are replaced with 50 cm\textsuperscript{-1} to correct for the breakdown in the harmonic oscillator approximation for low frequency vibrational modes.\cite{Ribeiro2011} The partitions functions are computed in the pMuTT\cite{LYM2019106864} Python package. From the partition functions, we compute $E^\text{ZP}$ and $F^\text{vib}$. A sample input file for the CP2K frequency calculations is provided in the Supporting Information.
...
\begin{figure}[H]
    ...
    \caption{
    Differential pair distribution functions (dPDFs) for experimental and simulated structures considered in this study. The color and numbering schemes are the same as in Figures~\ref{fig:phase_diagram_Ni_combined} and \ref{fig:Ni-structure-diagram}. The number in parenthesis is the average \ce{Ni} coordination number for the structures based on the interatomic distances.
    }
    \label{fig:dPDFs_TandP_trans_Ni}
\end{figure}

The results from dPDF analysis is shown in Figure~\ref{fig:dPDFs_TandP_trans_Ni}, where the dPDF of the experimentally observed structure is depicted with the black line (labeled Experimental in Figure~\ref{fig:dPDFs_TandP_trans_Ni}). Key peaks in the experimentally observed dPDF are the single peak at 2.02 {\AA}, split peaks at 3.01 {\AA} and 3.27 {\AA}, and split peaks at 3.78 {\AA} and 4.08 {\AA}. These correspond to \ce{Ni-O}, \ce{Ni{\Compactcdots}Ni}, and \ce{Ni{\Compactcdots}Zr} distances, respectively. Our analysis primarily focuses on the \ce{Ni-O} and \ce{Ni{\Compactcdots}Ni} seen in the experimental dPDF; we evaluate the performance of a structure on how well it recreates the features observed by the experimental dPDF. Now, the dPDFs shown in Figure~\ref{fig:dPDFs_TandP_trans_Ni} represent the structures that minimize the free energy according to \textit{ab initio} thermodynamic analysis. However, before focusing on the conditions where the structures are located, we first inspect the \ce{Ni} coordination numbers of the structures. \hl{Platero-Prats et al.} observed that the coordination number of the experimental dPDFs was thought to be 5-ish under activation conditions. We report the \ce{Ni} coordination numbers on the \hl{right side?} of Figure~\ref{fig:dPDFs_TandP_trans_Ni}. The structures that contain \ce{Ni} coordination numbers that deviate from the experimental observed coordination numbers, such as structures \hl{XXX}, \hl{XXX}, and \hl{XXX}, show little to no agreement in \ce{Ni-O} or \ce{Ni{\Compactcdots}Ni} peak positions. Therefore, we use coorindation numbers in defining the chemical potentials used in \textit{ab initio} thermodynamic analysis. 
