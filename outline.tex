...
\begin{itemize}
    \item Deposition of 3d transition metal atoms on MOF framework can occur in the vapor phase via atomic layer deposition (ALD) in MOFs (AIM) \hl{REFERENCE} and in condensed phase via solvothermal deposition in MOFs (SIM), \hl{REFERENCE} where the synthesis conditions regulate the active species material loading to generate more active site structures supported on the framework. Given the nature of the synthesis conditions, there is always a question about the precise structure of the active site for metal complexes supported on MOFs because characterization of the metal actives sites is suppressed by the bulk framework structure. Therefore, combined experimental and computational investigations suggest the synthesized structure of the active site. 
    
    
    A single-site model, comprised of a single metal species serving as the active site, has been shown to be 
    
    Further expanding on the single-site model, the work of \hl{REFERENCE HAFEERA} varied THE METAL SPECIES FROM A SINGLE NI-ATOM TO OTHER ATOMS TO STUDY THE XXXX REACTION ON THE STRUCTURE. DIfferences in the trends between experimental and theoretical modeling might stem the single-site model not being appropriate under the reaction conditions. 
    
    
    
    TWO GROWING MODELING APPRAOCHES HAVE EMERGED SINCE. 
    
    shows models of both single-site (mononuclear) active site structures  
    
    Different groups suggest the formation of a single-site active metal species (mononuclear) 
    \item \hl{REFERENCE-PLATERO-PRATS} used differential pair distribution function (PDF) analysis performed on experimental synthesized metal complex structures and computationally derived structures suggests the formation of a \ce{Ni4(OH)6} cluster attached to adjacent nodes within the c-pore of NU-1000.\hl{REFERENCE} The \ce{Ni4(OH)6} cluster 
