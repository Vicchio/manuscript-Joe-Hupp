...
Reaction conditions, such as temperature ($T$) and pressure ($P$), on a multinuclear \ce{Ni(II)} supported metal complex. The \ce{Ni(II)} complex (shown in Figure \ref{fig:ref_Ni4_structure}) is located within the small pore of NU-1000. Exposure to \ce{H2} gas induces structural and compositional changes to the cluster because of the presence of \ce{OH}- and \ce{M}-ligands. The \ce{OH}- and \ce{M}- ligands accept \ce{H} atoms from \ce{H2} adsorption and dissociation to generate \ce{H2O} and \ce{Ni-H} species, respectively. The generated adsorbed \ce{H2O} species can desorb into the gas phase. All the aforementioned transformations are dependent on the partial pressure of \ce{H2} ($P_{H_2}$), the partial pressure of \ce{H2O} ($P_{H_{2}O}$), and the reaction temperature ($T$). We model the dynamic changes in cluster structure and composition as a function of reaction conditions. 
...
Inclusion of reaction conditions required \textit{ab initio} thermodynamic analysis to compute the stability of the different structural and compositional variations by transforming the free energy from a fixed number of atoms to a fixed chemical potential. The multinuclear \ce{Ni(II)} complex features \ce{OH^{*}} and \ce{M^{*}} species and, when activated with molecular \ce{H2}, accept \ce{H} species on either \ce{OH^{*}} (forming waters, \ce{H2O}) or \ce{M^{*}} (forming metal hydrides, \ce{M-H}). The process of \ce{H2} adsorption and dissociation generates new structures that are compositional different relative to the reference structure. Here, we investigate structures that are compositionally different with respect to the number of \ce{H^{*}}, \ce{OH^{*}} and \ce{M^{*}} species relative to a reference structure. 
...
\end{equation}
Using Equation \ref{eq:reaction_equilibrium}, the relationship between the $\ce{H^{*}}$ and $\ce{OH^{*}}$ species and the gas phase \ce{H2} and \ce{H2O} conditions are shown in Equation \ref{eq:chemicalpotentialeq}. 
\begin{equation}
    \begin{split}
