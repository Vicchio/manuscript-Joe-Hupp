...
\hl{Check the bruce gates article that talks about the different nanocatalyst supports.. }
Metal-organic frameworks (MOFs) are an attractive support for single

NU-1000, comprised of \hl{XXXX} nodes connected by \hl{XXXX} linkers, is an attractive platform to support a plethora of metal complexes because of \hl{XXX}. The \ce{OH}-ligands dangling from the \ce{Zr} nodes provide an attaching point for 3d transition metals. Deposition of 3d transition metal atoms on MOF framework occurs in the vapor phase via atomic layer deposition (ALD) in MOFs (AIM) \hl{REFERENCE} and in condensed phase via solvothermal deposition in MOFs (SIM), \hl{REFERENCE}. The synthesis conditions regulate the active species loading to generate diverse active site structures supported on the framework. Characterization of the metal active sites is suppressed by the bulk framework structure, leading to questions about the structure and composition of the synthesized active site. Combined experimental and computational modeling investigates the active site structure for 3d transition metals supported on the standard NU-1000 MOF framework. 
    ...
    Determining the precise structure of the active site is further complicated by activation and reaction conditions induce structural and compositional changes on the cluster.\hl{REFERENCE}
     ...
     Therefore, combined experimental and computational investigations suggest the synthesized structure of the active site. \hl{Talk about how hard it was to determine the structure of the proton on the MOF framework... REFERENCE Planas 2014}
    
    Originally, the proposed active site structure consisted of a single metal attached to the MOF node.\hl{REFERENCE SINGLE-SITE MODEL} 
\begin{itemize}
    \item 
