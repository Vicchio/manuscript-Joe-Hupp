...
% There's the large review paper that is good for the introduction section. 
...
\begin{itemize}
    ...
    \begin{figure}
        \centering
        \includegraphics{zi-images/01-Ni-Graphics/2021-figure-structure-diagram-Ni-fixed.png}
        \caption{Schematic representation of the low energy structures appearing on the \ce{Ni(II)} phase diagram (Figure \ref{fig:phase_diagram_TandP_fixed_Ni}), with the naming and color scheme matching the phase diagram. \hl{The Ni...Ni and Ni...Zr distances are reported on each structure to provide insights into the configuration of the cluster.}}
...
\begin{itemize}
    \item Paragraph 1:
    \begin{itemize}
        \item Analysis suggests that ultimately the cluster is in a meta-stable state that is driven towards the breakdown of the cluster. When transforming with the number of \ce{Ni} atoms we see that the \ce{Ni} atoms are leeched from the support and accounted for by a bulk \ce{Ni} model.
        \item There is no observations about \ce{Ni(II)} leeching to form \ce{Ni(0)}-NPs in NU-1000 within the literature. However, there are discussions about the reduction of \ce{Cu(II)} to \ce{Cu(0)} under \ce{H2} reducing conditions at these temperatures. A consideration is the temperature of activation. The formation of \ce{Ni(0)}-NPs might occur at activation temperatures higher than the current standard activation conditions within the literature for MOF systems. 
        \item Question: If the cluster is driven towards not supporting any metal complex, then why is the metal complex supported on the MOF during synthesis? This could be explained from the synthesis conditions to create the supported metal complex onto the MOF.
        \item 
    \end{itemize}
    \item Paragraph 2:
    \begin{itemize}
        \item The suggested model structure is the light blue structure, which shows that the mu3-OH ligands are removed. Note that this structure does NOT show any Ni-H formation on the cluster. Also, comparing the work from \citeauthor{PlateroPrats2017} this structure contains under-coordinated \ce{Ni-atoms}.
        \item Thermodynamically, active site formation does not demonstrate the formation of a \ce{Ni-H}, which is often to be the active species here. Possible explanation is that the dominate species here does not the \ce{Ni-H}. 
        ...
        \item \ce{H2O} phase diagram suggests that at 50oC even with a small partial pressure of \ce{H2O} that the sample will be fully hydrated (from the \ce{H2O} partial pressure phase diagram.  
    \end{itemize}
    \item Paragraph 3:
    \begin{itemize}
